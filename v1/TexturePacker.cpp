...
public:
  ...
  virtual int packTextures(int &width,int &height,bool forcePowerOfTwo,bool onePixelBorder)  // pack the textures, the return code is the amount of wasted/unused area.
  {
    ...
    for (int i=0; i<mTextureCount; i++)
    {
      ...
      int y = t.mY+(t.mFlipped ? t.mWidth : t.mHeight);
      if ( y > height )
        height = y;
    }
...
class MyTexturePacker2 : public TexturePacker
{
	struct rect
	{
		int x,y,w,h;
	...
	std::map< std::pair<int,int> /*dimension*/, std::vector<rect> /*images*/ > columns;
	...
	public:
		...
		virtual void setTextureCount( int count )
		{}

		// add textures 0 - n
		virtual void addTexture( int w, int h )
		{
			bool rotated = false;
			...
			columns[ std::make_pair<w,h> ].push_back( rect() );
			columns[ std::make_pair<w,h> ].back().w = w;
			columns[ std::make_pair<w,h> ].back().h = h;
			columns[ std::make_pair<w,h> ].back().rotated = rotated;
		}

		// pack the textures, the return code is the amount of wasted/unused area.
		virtual int packTextures(int &width,int &height,bool forcePowerOfTwo,bool onePixelBorder) 
		{
